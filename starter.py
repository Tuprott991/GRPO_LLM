...
def _validate_numbers(equation_str: str, available_numbers: List[int]) -> bool:
    ...
    try:
        ...
        used_numbers = re.findall(r'\d+', equation_str)
        used_numbers = [int(n) for n in used_numbers]
        
        # Sort both lists to compare
        used_sorted = sorted(used_numbers)
        available_sorted = sorted(available_numbers)
        
        return used_sorted == available_sorted
    except:
        return False
...
def _evaluate_equation(equation_str: str) -> float | None:
    ...
    try:
        ...
        result = eval(equation_str, {"__builtins__": {}}, {})
        return float(result)
    except:
        return None
...
def reward_fn(generated_text: str, ground_truth: Dict) -> float:
    ...
    - 0.1: Partial - has <answer> tag but equation is incorrect
    - 0.0: Failed - no <answer> tag found
    """
    ...
    # Answer tag found, now check if it's correct
    # Validate numbers used
    if not _validate_numbers(equation, numbers):
        return 0.1
    ...
    # Check if result matches target (with small tolerance for float comparison)
...
def compute_loss(
    ...
    pi_ratio_clipped = torch.clamp(pi_ratio, 1.0 - clip_range, 1.0 + clip_range)
    clipped_term = advantages * pi_ratio_clipped
    
    # Step 4: Final loss (negative because we want to maximize)
    loss = -torch.minimum(unclipped_term, clipped_term)
    
    # Create a metadata dictionary to return, fulfilling the function's contract
    metadata = {
        "pi_ratio": pi_ratio.detach(),
        "unclipped_term": unclipped_term.detach(),
        "clipped_term": clipped_term.detach(),
    }
...
def masked_mean(tensor: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """
    Compute the mean of tensor values where mask=True for each row, 
    then average across the batch.
    """
    # Mask the tensor
    masked_tensor = tensor * mask
    
    # Sum over the sequence dimension and divide by number of True values per row
    sum_per_row = masked_tensor.sum(dim=1)
    count_per_row = mask.sum(dim=1).clamp(min=1)  # Avoid division by zero
    mean_per_row = sum_per_row / count_per_row
...
def masked_mean_drgrpo(tensor: torch.Tensor, mask: torch.Tensor, num_tokens: int) -> torch.Tensor:
    """
    Compute the sum of tensor values where mask=True, divided by num_tokens, 
    then average across the batch.
    ...
    # Sum over the sequence dimension and divide by num_tokens
    sum_per_row = masked_tensor.sum(dim=1)
    mean_per_row = sum_per_row / num_tokens
